...
setTimeout(() => {
  ...
  const externalDiff =
    parseFloat(afterOps.external) - parseFloat(initial.external);
  ...
  console.log(
    `External: ${externalDiff > 0 ? "+" : ""}${externalDiff.toFixed(1)}MB\n`,
  );
  ...
  const externalRatio =
    parseFloat(afterOps.external) / parseFloat(afterOps.heapUsed);
  if (externalRatio < 1.2) {
    ...
    console.log(
      "⚠️  WARNING: External/Heap ratio indicates potential external memory leak (>1.5x)",
    );
  }
  ...
  setTimeout(() => {
    ...
    const cleanupReduction =
      parseFloat(afterOps.heapUsed) - parseFloat(afterCleanup.heapUsed);
    console.log(`🗑️  Heap memory freed: ${cleanupReduction.toFixed(1)}MB`);

    console.log("\n✅ Memory test completed!");
    console.log("=========================");
    console.log(
      "Compare these results with the original 178MB RSS to verify fixes.",
    );
    process.exit(0);
  }, 2000);
}, 1000);
