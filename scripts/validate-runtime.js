...
class RuntimeValidator {
  ...
  loadConfig() {
    try {
      const configPath = path.join(process.cwd(), 'runtime-validation.config.json');
  ...
  async loadSchemas() {
    ...
    for (const { name, path: schemaPath } of schemaFiles) {
      try {
        ...
        await this.log(`✗ Failed to load schema ${name}: ${error.message}`, 'error');
        throw error;
      }
    }
  }
  ...
  async validateApiEndpoints() {
    ...
    const testEndpoints = [
      ...
      { path: '/api/dashboard/analytics', method: 'GET', expectSchema: 'api.analytics.dashboard.response' },
      { path: '/api/calls', method: 'GET', expectSchema: 'api.calls.list.response' }
    ];
    ...
    for (const endpoint of testEndpoints) {
      ...
      try {
        ...
        const isValid = this.validateApiResponse(response, endpoint.expectSchema);
  ...
  validateData(schemaName, data, definition = null) {
    try {
      ...
      if (definition) {
        ...
        if (!defSchema) {
          throw new Error(`Definition ${definition} not found in schema ${schemaName}`);
        }
      ...
      if (!validateFn) {
        throw new Error(`Cannot create validator for ${schemaName}${definition ? `.${definition}` : ''}`);
      }
  ...
  async generateReport(results) {
    ...
    if (this.config.reporting.format === 'json' || this.config.reporting.format === 'all') {
      const jsonPath = path.join(this.reportDir, `validation-${timestamp}.json`);
    ...
    if (this.config.reporting.format === 'html' || this.config.reporting.format === 'all') {
      const htmlPath = path.join(this.reportDir, `validation-${timestamp}.html`);
    ...
    if (this.config.reporting.format === 'text' || this.config.reporting.format === 'all') {
  ...
  generateSummary(results) {
    ...
    if (results.database) {
      Object.keys(results.database).forEach(category => {
        ...
        summary.categories[category] = {
          ...
          errorRate: ((data.invalid / (data.valid + data.invalid)) * 100).toFixed(2)
        };
      });
    }
    ...
    if (results.api) {
      ...
      summary.categories.api = {
        ...
        errorRate: ((results.api.summary.failed / results.api.summary.total) * 100).toFixed(2)
      };
    }
    
    summary.successRate = summary.totalValidations > 0 
      ? (((summary.totalValidations - summary.totalErrors) / summary.totalValidations) * 100).toFixed(2)
      : 100;
...
<body>
    ...
    <div class="section">
        ...
        <table>
            ...
            ${Object.entries(reportData.summary.categories).map(([category, data]) => `
            <tr>
                <td>${category}</td>
                <td>${data.total}</td>
                <td>${data.valid}</td>
  ...
  generateTextReport(reportData) {
    ...
    Object.entries(reportData.summary.categories).forEach(([category, data]) => {
  ...
  async startMonitoring() {
    ...
    await this.log(`Starting continuous monitoring (interval: ${this.config.monitoring.interval}ms)...`);
    
    const interval = setInterval(async () => {
      try {
        await this.runValidation();
      } catch (error) {
        await this.log(`Monitoring validation failed: ${error.message}`, 'error');
      }
