...
class ChangeWizard {
  ...
  async loadRegistry() {
    try {
      const registryPath = path.join(process.cwd(), 'config/ssot-registry.json');
      const content = await fs.readFile(registryPath, 'utf8');
      this.registry = JSON.parse(content);
    } catch (error) {
      console.error(chalk.red('✗ Failed to load SSOT registry:', error.message));
      process.exit(1);
    }
  }
  ...
  displayWelcome() {
    ...
    console.log(chalk.gray('Let me guide you through making changes to the system\n'));
  }
  ...
  async handleDatabaseChange() {
    ...
    const subChoice = await this.promptUser(chalk.cyan('Select option (1-5): '));
    ...
    console.log(chalk.white('  2. npm run generate:types           # Generate new TypeScript types'));
    console.log(chalk.white('  3. npm run check:breaking-changes   # Check for breaking changes'));
    
    if (subChoice === '3') {
      console.log(chalk.red('  4. npm run generate:migration-guide # REQUIRED for breaking changes'));
    }
    
    console.log(chalk.white('  4. npm run validate:ssot            # Validate all consistency'));
    
    console.log(chalk.yellow('\nSECONDARY FILES TO UPDATE:'));
    console.log(chalk.white('  • tools/migrations/[new-migration].sql'));
    console.log(chalk.white('  • packages/shared/types/core.ts (auto-generated)'));
    console.log(chalk.white('  • schemas/dashboard-schema.json (if UI changes needed)'));
    
    if (subChoice === '1' || subChoice === '2') {
      console.log(chalk.yellow('\nIF ADDING NEW API ENDPOINTS:'));
      console.log(chalk.white('  • apps/server/routes/[relevant-route].ts'));
      console.log(chalk.white('  • apps/client/src/services/[relevant-service].ts'));
      console.log(chalk.white('  • npm run generate:api-docs'));
    }

    await this.showTestingChecklist(['Database migration', 'Type checking', 'API endpoints']);
  }
  ...
  async handleApiChange() {
    ...
    console.log(chalk.blue('PRIMARY FILE TO CHANGE:'));
    console.log(chalk.white('  apps/server/routes/[choose appropriate route file]'));
    ...
    console.log(chalk.white('  2. npm run generate:api-docs         # Update API documentation'));
    console.log(chalk.white('  3. npm run validate:runtime --api    # Validate API schema'));
    
    if (subChoice === '3' || subChoice === '4') {
      console.log(chalk.red('  4. npm run check:breaking-changes    # REQUIRED for breaking changes'));
    }
    
    console.log(chalk.yellow('\nSECONDARY FILES TO UPDATE:'));
    console.log(chalk.white('  • schemas/api-schema.json (if request/response format changes)'));
    console.log(chalk.white('  • packages/shared/types/api.ts (auto-generated)'));
    console.log(chalk.white('  • apps/client/src/services/[relevant-service].ts'));
    console.log(chalk.white('  • docs/API_DOCUMENTATION.md (auto-generated)'));

    await this.showTestingChecklist(['API endpoint testing', 'Frontend integration', 'Authentication']);
  }
  ...
  async handleFrontendChange() {
    ...
    console.log(chalk.white('  apps/client/src/components/[choose appropriate directory]'));
    console.log(chalk.gray('    • ui/ - Reusable UI components'));
    console.log(chalk.gray('    • dashboard/ - Dashboard-specific components'));
    console.log(chalk.gray('    • unified-dashboard/ - Unified dashboard components'));
    console.log(chalk.gray('    • pages/ - Page-level components'));
    
    console.log(chalk.yellow('\nAUTOMATION STEPS:'));
    console.log(chalk.white('  1. Create/modify your component files'));
    console.log(chalk.white('  2. npm run type-check                # TypeScript validation'));
    
    if (subChoice === '5') {
      console.log(chalk.white('  3. npm run validate:runtime --components # Validate form schemas'));
    }
    
    console.log(chalk.yellow('\nSECONDARY FILES TO UPDATE:'));
    console.log(chalk.white('  • packages/shared/types/ui.ts (if new props/interfaces)'));
    console.log(chalk.white('  • apps/client/src/components/index.ts (export new components)'));
    
    if (subChoice === '5') {
      console.log(chalk.white('  • schemas/dashboard-schema.json (add validation schema)'));
    }
    ...
    await this.showTestingChecklist(['Component rendering', 'User interactions', 'Responsive design']);
  }
  ...
  async handleConfigChange() {
    ...
    console.log(chalk.white('  2. npm run sync:changes              # Sync to all environments'));
    console.log(chalk.white('  3. npm run validate:runtime --config # Validate configuration'));

    await this.showTestingChecklist(['Build process', 'Environment validation', 'Deployment']);
  }
  ...
  async handleHotelFeatures() {
    ...
    switch (subChoice) {
      case 1:
        ...
        console.log(chalk.white('  Frontend: apps/client/src/components/EmailForm.tsx'));
        console.log(chalk.white('  Service: apps/server/services/orderService.ts'));
        break;
      case 2:
        console.log(chalk.blue('VOICE ASSISTANT:'));
        console.log(chalk.white('  Config: apps/client/src/context/AssistantContext.tsx'));
        console.log(chalk.white('  Service: apps/server/services/vapiIntegration.ts'));
        console.log(chalk.white('  Frontend: apps/client/src/components/VoiceAssistant.tsx'));
        break;
      case 4:
        console.log(chalk.blue('MULTI-TENANT FEATURES:'));
        console.log(chalk.white('  Database: tenants table in schema.ts'));
        console.log(chalk.white('  Middleware: apps/server/middleware/tenant.ts'));
        console.log(chalk.white('  Service: apps/server/services/tenantService.ts'));
        break;
    }

    await this.showTestingChecklist(['Feature functionality', 'Multi-tenant isolation', 'Voice assistant integration']);
  }
  ...
  async handleVoiceAssistant() {
    ...
    console.log(chalk.white('  • apps/client/src/context/AssistantContext.tsx'));
    console.log(chalk.white('  • apps/server/services/vapiIntegration.ts'));
    console.log(chalk.white('  • apps/client/src/components/VoiceAssistant.tsx'));
    ...
    if (subChoice === 4) {
      ...
      console.log(chalk.white('  1. Add new language file: apps/client/src/i18n/[lang].json'));
    ...
    await this.showTestingChecklist(['Voice commands', 'Language switching', 'Call quality', 'Response accuracy']);
  }
  ...
  async showTestingChecklist(testAreas) {
    ...
    console.log(chalk.cyan('  Run: npm run validate:ssot     # System consistency'));
    console.log(chalk.cyan('  Run: npm run validate:runtime  # Runtime validation'));
  }

  async generateChangeChecklist() {
    const taskDescription = await this.promptUser(chalk.cyan('\nDescribe your change in one sentence: '));
    ...
    while (true) {
      ...
      switch (choice) {
        ...
        case 5:
          ...
          console.log(chalk.white('  Run: npm run update:readme           # Update README'));
          console.log(chalk.white('  Run: npm run generate:api-docs       # Update API docs'));
          console.log(chalk.white('  Run: npm run generate:changelog      # Generate changelog'));
          break;
        case 6:
          console.log(chalk.red('\n🔄 Breaking Changes - CAREFUL!'));
          console.log(chalk.white('  1. Make your changes'));
          console.log(chalk.white('  2. npm run check:breaking-changes   # REQUIRED'));
          console.log(chalk.white('  3. npm run generate:migration-guide # REQUIRED'));
          console.log(chalk.white('  4. npm run generate:migration-docs  # REQUIRED'));
          console.log(chalk.yellow('  5. Review generated migration guide before deploying'));
          break;
        case 7:
        ...
        case 8:
          ...
          console.log(chalk.white('  Database: staff table in packages/shared/db/schema.ts'));
          console.log(chalk.white('  Middleware: apps/server/middleware/auth.ts'));
          console.log(chalk.white('  Frontend: apps/client/src/context/AuthContext.tsx'));
          break;
        case 9:
          console.log(chalk.blue('\n📊 Analytics & Reporting:'));
          console.log(chalk.white('  API: apps/server/routes/analytics.ts'));
          console.log(chalk.white('  Service: apps/server/services/analyticsService.ts'));
          console.log(chalk.white('  Frontend: apps/client/src/pages/AnalyticsDashboard.tsx'));
          break;
        case 10:
      ...
      const continueChoice = await this.promptUser(chalk.cyan('\nNeed help with another change? (y/n): '));
