...
async function quickMemoryTest() {
  ...
  while (Date.now() - startTime < testDuration) {
    try {
      ...
      if (runtime % 30 === 0 || runtime < 30) {
        console.log(
          `⏰ ${runtime}s | RSS: ${memory.rss}MB | Heap: ${memory.heap}MB | External: ${memory.external}MB | Requests: ${requestCount}`,
        );
      }
      ...
      console.warn(
        `⚠️ Error at ${Math.round((Date.now() - startTime) / 1000)}s:`,
        error.message,
      );
      await new Promise((resolve) => setTimeout(resolve, 10000));
    }
  ...
  if (memorySnapshots.length >= 2) {
    ...
    console.log(
      `💾 RSS: ${first.rss}MB → ${last.rss}MB (${rssGrowth > 0 ? "+" : ""}${rssGrowth}MB)`,
    );
    console.log(
      `💾 External: ${first.external}MB → ${last.external}MB (${externalGrowth > 0 ? "+" : ""}${externalGrowth}MB)`,
    );
    ...
    if (Math.abs(rssGrowthRate) < 0.5 && Math.abs(extGrowthRate) < 1.0) {
      ...
      console.log(
        "❌ STILL LEAKING - Native modules still growing significantly",
      );
      console.log(
        "💡 Need additional investigation or more aggressive cleanup",
      );
      return 1;
    }
  } else {
