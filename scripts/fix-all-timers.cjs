...
function hasTimerManager(content) {
  return content.includes('TimerManager') || 
...
function hasNativeTimers(content) {
  return /(?<!TimerManager\.)setInterval\s*\(/.test(content) || 
...
function replaceTimers(content) {
  ...
  const setIntervalRegex = /(?<!TimerManager\.)setInterval\s*\(\s*([^,]+),\s*([^,)]+)(?:,([^)]*))?\)/g;
  content = content.replace(setIntervalRegex, (match, callback, delay, name) => {
    modified = true;
    stats.setIntervalFixed++;
    
    // Generate a name if not provided
    const timerName = name ? name.trim() : `"auto-generated-interval-${stats.setIntervalFixed}"`;
  ...
  const setTimeoutRegex = /(?<!Promise\(.*resolve.*\))(?<!TimerManager\.)setTimeout\s*\(\s*([^,]+),\s*([^,)]+)(?:,([^)]*))?\)/g;
  content = content.replace(setTimeoutRegex, (match, callback, delay, name) => {
    ...
    // Generate a name if not provided
    const timerName = name ? name.trim() : `"auto-generated-timeout-${stats.setTimeoutFixed}"`;
