...
const MigrationDocsConfigSchema = z.object({
  ...
  detection: z.object({
    ...
    apiFiles: z.array(z.string()).default([
...
class MigrationDocsGenerator {
  ...
  async detectDatabaseChanges() {
    ...
    try {
      ...
      await this.log(`Database change detection error: ${error.message}`, 'warn');
    }
  }
    ...
    try {
      ...
      if (await this.fileExists(routesDir)) {
        ...
        for (const file of files) {
          if (file.endsWith('.ts')) {
            const changes = await this.getFileChanges(path.join(routesDir, file));
            if (changes.length > 0) {
              const apiChange = {
  ...
  async detectConfigurationChanges() {
    ...
    try {
      ...
      await this.log(`Configuration change detection error: ${error.message}`, 'warn');
    }
  }
  ...
  async detectDependencyChanges() {
    ...
    try {
      ...
      if (packageChanges.length > 0) {
        ...
        try {
          const previousContent = execSync('git show HEAD~1:package.json', { encoding: 'utf8' });
          previousPackageJson = JSON.parse(previousContent);
        } catch {
          // No previous version or file doesn't exist
        }
        
        const depChanges = this.compareDependencies(previousPackageJson, packageJson);
      ...
      await this.log(`Dependency change detection error: ${error.message}`, 'warn');
    }
  }
  ...
  async detectBreakingChanges() {
    ...
    try {
      ...
      const output = execSync('node scripts/check-breaking-changes.js --format=json', {
        encoding: 'utf8',
        stdio: 'pipe'
      });
      ...
      await this.log(`Breaking change detection warning: ${error.message}`, 'warn');
    }
  }
  ...
  generateDependencySteps() {
    ...
    for (const depChange of this.changeCategories.dependencies) {
      if (depChange.changes.major.length > 0) {
        steps.push('⚠️ Major version updates detected - review breaking changes');
  ...
  async generateMarkdownDoc() {
    ...
    // Risk assessment
    doc += '## Risk Assessment\n\n';
    const hasHighRisk = Object.values(this.changeCategories).flat().some(c => c.impact === 'high');
    const requiresDowntime = Object.values(this.changeCategories).flat().some(c => c.requiresDowntime);
    ...
    if (requiresDowntime) {
      doc += '⏰ **DOWNTIME REQUIRED** - System will be unavailable during migration\n\n';
    } else {
      doc += '✅ **ZERO DOWNTIME** - Migration can be performed without service interruption\n\n';
    }
    ...
    doc += '---\n\n';
    doc += '*This migration guide was automatically generated by the SSOT system.*\n';
  ...
  async generateJsonDoc() {
    return JSON.stringify({
      ...
      summary: {
        ...
        requiresDowntime: Object.values(this.changeCategories).flat().some(c => c.requiresDowntime),
        riskLevel: Object.values(this.changeCategories).flat().some(c => c.impact === 'high') ? 'high' : 'medium'
      },
...
<body>
    ${markdownContent
      ...
      .replace(/## /g, '<h2>')
      .replace(/# /g, '<h1>')
      .replace(/🔴 \*\*HIGH RISK\*\*/g, '<div class="risk-high">🔴 <strong>HIGH RISK</strong>')
      .replace(/🟡 \*\*MEDIUM RISK\*\*/g, '<div class="risk-medium">🟡 <strong>MEDIUM RISK</strong>')
      .replace(/⏰ \*\*DOWNTIME REQUIRED\*\*/g, '<div class="downtime">⏰ <strong>DOWNTIME REQUIRED</strong>')
  ...
  async saveMigrationDoc(content, format) {
    ...
    await this.log(`✓ ${format.toUpperCase()} migration guide saved: ${filePath}`, 'success');
    return filePath;
  }
  ...
  async generate() {
    ...
    try {
      ...
      const totalChanges = Object.values(this.changeCategories).flat().length;
      if (totalChanges === 0) {
        await this.log('No changes detected - skipping migration guide generation', 'info');
      ...
      await this.log('✓ Migration documentation generation completed', 'success');
      ...
      await this.log(`✗ Migration documentation generation failed: ${error.message}`, 'error');
      throw error;
    }
  }
}
...
Examples:
  ...
  try {
    ...
    if (result.outputs.length > 0) {
      ...
      console.log(`  Estimated duration: ${result.summary.estimatedDuration} minutes`);
      console.log(`  Risk level: ${result.summary.riskLevel}`);
      console.log(`  Requires downtime: ${result.summary.requiresDowntime ? 'Yes' : 'No'}`);
