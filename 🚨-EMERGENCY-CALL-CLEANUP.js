...
function forceEndCall() {
    ...
    try {
        ...
        const endButton = document.querySelector('[data-testid="end-call"]') ||
...
function stopRequestSpam() {
    ...
    window.fetch = function (...args) {
        ...
        if (url && (url.includes('minhonmuine') || url.includes('/api/'))) {
            if (blockedCount < 10) { // Block first 10 requests to stop spam
                blockedCount++;
...
function resetCallState() {
    ...
    try {
        ...
        Object.keys(localStorage).forEach(key => {
            if (key.includes('call') || key.includes('vapi') || key.includes('voice')) {
