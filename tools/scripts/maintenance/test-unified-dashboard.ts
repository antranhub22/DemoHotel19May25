...
class UnifiedDashboardTester {
    ...
    for (const user of TEST_USERS) {
      ...
      try {
        ...
        if (response.ok && data.success && data.token) {
          console.log(`✅ ${user.description}: Login successful (${responseTime}ms)`);
    ...
    for (const user of TEST_USERS) {
      ...
      for (const page of DASHBOARD_PAGES) {
        ...
        try {
          ...
          const apiPath = page.path.replace('/unified-dashboard', '/api/dashboard');
          ...
          const accessText = shouldHaveAccess ? 'SHOULD ACCESS' : 'SHOULD BLOCK';
          
          console.log(`${accessIcon} ${page.name.padEnd(20)} | ${accessText.padEnd(12)} | ${apiTestResult} | ${responseTime}ms`);
  ...
  async testDataFlows(): Promise<void> {
    ...
    const apiTests = [
      ...
      { name: 'Service Distribution', endpoint: '/api/analytics/service-distribution', method: 'GET' },
      { name: 'Hourly Activity', endpoint: '/api/analytics/hourly-activity', method: 'GET' },
      { name: 'Staff Requests', endpoint: '/api/staff/requests', method: 'GET' }
    ];
    ...
    for (const test of apiTests) {
      ...
      try {
        ...
        if (response.ok) {
          console.log(`✅ ${test.name.padEnd(20)} | Status: ${response.status} | Time: ${responseTime}ms`);
        } else {
          console.log(`❌ ${test.name.padEnd(20)} | Status: ${response.status} | Error: ${data.error || data.message}`);
        }
      } catch (error) {
  ...
  generateReport(): void {
    ...
    const resultsByRole = this.results.reduce((acc, result) => {
    ...
    if (failedResults.length > 0) {
      ...
      failedResults.forEach(result => {
        console.log(`   ${result.user} (${result.role}) -> ${result.page}: ${result.error || 'Access denied'}`);
      });
    }
