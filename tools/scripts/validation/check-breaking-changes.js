...
class BreakingChangesDetector {
  ...
  async detectBreakingChanges() {
    ...
    try {
      ...
      console.log(
        `📍 Comparing ${this.previousCommit} → ${this.currentCommit}`
      );
    ...
    try {
      ...
      for (const change of changes) {
        if (this.isBreakingSchemaChange(change)) {
          ...
          this.addPotentialBreakingChange(
            'schema',
            change.type,
            change.description,
            change.impact
  ...
  parseSchemaChanges(diff) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (isRemoval || isAddition) {
        ...
        if (
          cleanLine.includes('.notNull()') ||
          cleanLine.includes('.nullable()')
        ) {
          const nullabilityType = cleanLine.includes('.notNull()')
            ? 'not_null'
            : 'nullable';
          changes.push({
  ...
  getFieldAdditionImpact(fieldDefinition) {
    ...
    if (
      fieldDefinition.includes('.notNull()') &&
      !fieldDefinition.includes('.default(')
    ) {
      return 'HIGH';
    }
  ...
  isBreakingSchemaChange(change) {
    ...
    return (
      breakingTypes.includes(change.type) ||
      (change.type === 'field_added' && change.impact === 'HIGH')
    );
  }

  isPotentiallyBreakingSchemaChange(change) {
    const potentialTypes = ['foreign_key_added', 'index_removed'];
  ...
  async checkValidationChanges() {
    ...
    try {
      ...
      console.log(
        `  ⚠️ Could not analyze validation changes: ${error.message}`
      );
    }
  }
  ...
  isPotentiallyBreakingValidationChange(change) {
    ...
    return (
      ...
      ['string', 'number', 'boolean'].includes(change.validationType)
    );
  }
  ...
  parseApiChanges(diff, fileName) {
    ...
    for (const line of lines) {
      ...
      if (isRemoval || isAddition) {
        ...
        const endpointMatch = cleanLine.match(
        ...
        if (
          cleanLine.includes('verifyJWT') ||
          cleanLine.includes('checkLimits') ||
          cleanLine.includes('requireFeature')
        ) {
          changes.push({
  ...
  isBreakingApiChange(change) {
    const breakingTypes = ['endpoint_removed', 'middleware_removed'];
  ...
  getFileDiff(filePath) {
    try {
      ...
      const diff = execSync(
        `git diff ${this.previousCommit} HEAD -- "${relativePath}"`,
        {
          encoding: 'utf8',
  ...
  async generateReport() {
    const report = {
      ...
      summary: {
        ...
        totalChanges:
          this.breakingChanges.length +
          this.potentialBreakingChanges.length +
          this.safeChanges.length,
      },
      analysis: {
        hasBreakingChanges: this.breakingChanges.length > 0,
        requiresMigrationGuide: this.breakingChanges.some(
          c => c.migrationComplexity === 'HIGH'
        ),
        affectedSystems: [
          ...new Set(
            this.breakingChanges.flatMap(c => c.affectedSystems || [])
          ),
        ],
      },
      changes: {
  ...
  printSummary() {
    ...
    console.log(
      `🔍 Comparison: ${this.previousCommit} → ${this.currentCommit}`
    );
    console.log(`❌ Breaking Changes: ${this.breakingChanges.length}`);
    console.log(
      `⚠️  Potentially Breaking: ${this.potentialBreakingChanges.length}`
    );
    ...
    if (this.breakingChanges.length > 0) {
      ...
      this.breakingChanges.forEach((change, index) => {
        console.log(
          `${index + 1}. [${change.category}] ${change.description} (${change.impact})`
        );
    ...
    if (this.breakingChanges.length > 0) {
      console.log(
        '🚨 Breaking changes detected! Consider generating a migration guide.'
      );
    } else {
