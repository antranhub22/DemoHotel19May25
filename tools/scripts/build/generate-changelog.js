...
const ChangelogConfigSchema = z.object({
  ...
  categories: z
    ...
    .default([
      ...
      {
        name: 'Dependencies',
        keywords: ['deps', 'dependency', 'update'],
      ...
      {
        name: 'Database',
        keywords: ['db', 'database', 'schema', 'migration'],
      ...
      },
      {
        ...
        description: 'API changes and updates',
        emoji: '🔌',
      },
    ]),
  versioning: z.object({
...
class ChangelogGenerator {
  ...
  async getSchemaChanges() {
    ...
    try {
      ...
      await this.log(
        `Error detecting schema changes: ${error.message}`,
        'warn'
      );
      return [];
    }
  ...
  async detectBreakingChanges() {
    ...
    try {
      ...
      const output = execSync(
        'node scripts/check-breaking-changes.js --check-only',
        {
          encoding: 'utf8',
          stdio: 'pipe',
        }
      ...
      await this.log(
        `Could not detect breaking changes: ${error.message}`,
        'warn'
      );
      return [];
    }
  ...
  async generateHtmlChangelog(version, schemaChanges) {
    const markdownContent = await this.generateMarkdownChangelog(
      version,
      schemaChanges
    );
...
<body>
  ...
  async saveChangelog(content, format, version) {
    ...
    await this.log(
      `✓ ${format.toUpperCase()} changelog saved: ${filePath}`,
      'success'
    );
    return filePath;
  }
  ...
  async generate() {
    ...
    try {
      ...
      const hasBreaking =
        this.breakingChanges.length > 0 || breakingChanges.length > 0;
      ...
      if (
        ...
        const markdown = await this.generateMarkdownChangelog(
          nextVersion,
          schemaChanges
        );
        const path = await this.saveChangelog(
          markdown,
          'markdown',
          nextVersion
        );
      ...
      if (
        ...
        const json = await this.generateJsonChangelog(
          nextVersion,
          schemaChanges
        );
      ...
      if (
        ...
        const html = await this.generateHtmlChangelog(
          nextVersion,
          schemaChanges
        );
