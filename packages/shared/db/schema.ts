import { pgTable, text, integer, real, numeric, timestamp, serial, primaryKey, boolean } from "drizzle-orm/pg-core";
import { sql } from "drizzle-orm";
import { createInsertSchema } from "drizzle-zod";
import { z } from "zod";

// ==============================================================
// PostgreSQL-Only Database Schema - Simplified & Consistent
// ==============================================================
...
export const call_summaries = pgTable("call_summaries", {
  id: serial("id").primaryKey(),
  call_id: text("call_id").notNull(),
  content: text("content").notNull(),
  timestamp: timestamp("timestamp").default(sql`CURRENT_TIMESTAMP`), // ✅ PostgreSQL TIMESTAMP
...
export const insertCallSummarySchema = createInsertSchema(call_summaries).extend({
  timestamp: z.union([
    z.date(),
    z.string().datetime(),
    z.number().transform(val => new Date(val))
