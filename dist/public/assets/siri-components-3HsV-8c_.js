import { r as m, j as p } from './react-core-C6DwaHZM.js';
import { a5 as X } from './components-CjbIaAhs.js';
import { a as Y, j as W } from './hooks-context-CVvU1W40.js';
class f {
  static {
    this.DEBUG_LEVEL = 0;
  }
  static setDebugLevel(e) {
    ((f.DEBUG_LEVEL = e),
      console.log(
        `[DebugManager] Debug level set to: ${e} (0: off, 1: errors only, 2: all)`
      ));
  }
  ...
  log(e, ...t) {
    f.DEBUG_LEVEL >= 2 && console.log(`[SiriButton] ${e}`, ...t);
  }
  debug(e, ...t) {
    f.DEBUG_LEVEL >= 2 && console.log(`[SiriButton] ${e}`, ...t);
  }
  warn(e, ...t) {
    f.DEBUG_LEVEL >= 1 && console.warn(`[SiriButton] ${e}`, ...t);
  }
  debugWarn(e, ...t) {
    f.DEBUG_LEVEL >= 1 && console.warn(`[SiriButton] ${e}`, ...t);
  }
  error(e, ...t) {
    console.error(`[SiriButton] ${e}`, ...t);
  }
  debugError(e, ...t) {
    console.error(`[SiriButton] ${e}`, ...t);
  }
...
typeof window < 'u' &&
  ((window.SiriDebugControls = {
    setLevel: l => {
      (f.setDebugLevel(l), console.log(`🔧 Voice debug level set to: ${l}`));
    },
...
class q {
  ...
  drawBaseCircle(e) {
    ...
    this.ctx.save();
    const h = n * (r ? 1.08 : a ? 1.04 : 1),
      u = this.ctx.createRadialGradient(t, i, 0, t, i, h);
    (u.addColorStop(0, s.primary + '95'),
      u.addColorStop(0.7, s.secondary + '60'),
      u.addColorStop(1, s.glow + '20'),
      this.ctx.beginPath(),
      this.ctx.arc(t, i, h + 15, 0, Math.PI * 2),
      (this.ctx.fillStyle = s.glow + '30'),
      (this.ctx.filter = 'blur(8px)'),
      this.ctx.fill(),
      (this.ctx.filter = 'none'),
      this.ctx.beginPath(),
      this.ctx.arc(t, i, h, 0, Math.PI * 2),
    ...
    (o.addColorStop(0, 'rgba(255, 255, 255, 0.4)'),
      o.addColorStop(1, 'rgba(255, 255, 255, 0)'),
      this.ctx.beginPath(),
      this.ctx.arc(t, i, h * 0.8, 0, Math.PI * 2),
  ...
  drawWaveform(e) {
    ...
    this.ctx.save();
    for (let c = 0; c < 3; c++) {
      const h = n * (1.15 + c * 0.1),
        u = 32;
      this.ctx.beginPath();
      for (let o = 0; o <= u; o++) {
        const y = (o / u) * Math.PI * 2,
          b = Math.sin(r + o * 0.3 + c * 1.5) * (8 + a * 12),
          x = h + b,
          S = t + Math.cos(y) * x,
          v = i + Math.sin(y) * x;
        o === 0 ? this.ctx.moveTo(S, v) : this.ctx.lineTo(S, v);
      }
      (this.ctx.closePath(),
  ...
  drawParticles(e) {
    ...
    for (let n = t.length - 1; n >= 0; n--) {
      ...
      const a = Math.floor(255 * r.alpha)
        .toString(16)
        .padStart(2, '0');
      ((this.ctx.fillStyle = i.secondary + a),
  ...
  drawTimeRing(e) {
    const {
      ...
      c = Math.min(1, r / a),
      h = c < 0.5 ? s.secondary : c < 1 ? s.primary : '#e53935';
    (this.ctx.save(),
      this.ctx.beginPath(),
      this.ctx.arc(
        t,
  ...
  drawTimeText(e) {
    ...
    (this.ctx.save(),
      ...
      (this.ctx.shadowBlur = 8),
      (this.ctx.fillStyle = a ? '#e8e8e8' : '#fff'),
      (this.ctx.globalAlpha = 0.95),
      this.ctx.fillText(u, t, i + n * 1.45),
      this.ctx.restore());
  }
  drawIdleFlash(e) {
    const {
    ...
    const h = 0.7 + 0.3 * Math.abs(Math.sin(a * 1.2));
    (this.ctx.save(),
      (this.ctx.globalAlpha = 0.18 * h),
      this.ctx.beginPath(),
  ...
  calculateDimensions() {
    ...
    try {
      ...
      const i = Math.max(200, t.width || 400),
        ...
        c = { width: i, height: n, centerX: r, centerY: a, radius: s };
...
class R {
  constructor(e, t) {
    ((this.isInitialized = !1),
      (this.debug = new f()),
      (this.emergency = new G(this.debug)),
      (this.renderer = new q(null, this.debug)),
      (this.animation = new U(this.debug, this.emergency, this.renderer)),
      (this.dimensions = new K(this.debug, this.emergency)),
      (this.state = new J(this.debug)),
...
typeof window < 'u' &&
  ((window.SiriDebugControls = {
    setLevel: l => {
      (R.setDebugLevel(l), console.log(`🔧 Voice debug level set to: ${l}`));
    },
    getLevel: () => R.getDebugLevel(),
    silent: () => R.setDebugLevel(0),
    errorsOnly: () => R.setDebugLevel(1),
    verbose: () => R.setDebugLevel(2),
    help: () => {
      console.log(`
🔧 SiriDebugControls Help:
- SiriDebugControls.silent()     -> Turn off all debug logs
...
- Enhanced mobile debugging support
  ...
  (window.voiceDebugOff = () => R.setDebugLevel(0)),
  (window.voiceDebugOn = () => R.setDebugLevel(2)));
const Q = ({ isListening: l, volumeLevel: e, colors: t, size: i = 280 }) => {
    ...
    (m.useEffect(() => {
      ...
      m.useEffect(() => {
        if (l) {
          const o = setInterval(() => {
            s(y => [...y, { id: Date.now(), scale: 1, opacity: 0.4 }]);
          }, 300);
      ...
      children: [
        ...
        p.jsx('div', {
          style: {
            ...
            transform: `scale(${u})`,
            boxShadow: `0 0 ${l ? 40 : 20}px ${t.glow}`,
  ...
  ee = ({
    ...
    const s = (g, ...d) => {},
      c = (g, ...d) => {},
      h = (g, ...d) => {
        console.error(`[SiriCallButton] ${g}`, ...d);
      },
      u = m.useRef(null),
      o = m.useRef(!1),
      [y, b] = m.useState('idle'),
      ...
      M = m.useCallback(() => {
        if (!o.current) {
          ...
          try {
            ...
            if (g)
              try {
                g.querySelectorAll('canvas').forEach(D => {
                  D.parentElement &&
                    document.contains(D) &&
                    D.parentElement.removeChild(D);
                });
              } catch (d) {
                h('Failed to remove canvases:', d);
              }
            S(!1);
          } catch (g) {
            h('Safe cleanup failed:', g);
          }
        }
      }, [t]),
      z = m.useCallback(() => {
        ...
        try {
          if (u.current)
            try {
              typeof u.current.emergencyStopPublic == 'function' &&
                u.current.emergencyStopPublic();
            } catch (g) {
              h('Failed to emergency stop SiriButton:', g);
            }
          (M(), c('🚨 EMERGENCY STOP COMPLETED'));
        } catch (g) {
          h('Emergency stop failed:', g);
        }
      }, [M]),
      I = m.useCallback((g, d) => {
        (u.current &&
          (u.current.setInteractionMode('active'),
          d && u.current.setTouchPosition(d.x, d.y)),
          s('🎯 [SiriCallButton] Interaction start:', { position: d }));
      }, []),
      F = m.useCallback(
        async g => {
          if (
            (s(),
            s('  🎯 Event type:', g.type),
            s('  🎯 Event target:', g.target),
            u.current && (u.current.setInteractionMode('idle'), s()),
            v.current)
          ) {
            s();
            return;
          ...
          try {
            if (!l && i) {
              ...
              try {
                ...
                (h('🎤 [SiriCallButton] ❌ onCallStart() error:', d),
                  b('idle'));
              }
            } else
              l && n
      ...
      L = m.useCallback(g => {
        u.current && u.current.setInteractionMode(g ? 'hover' : 'idle');
      }, []);
    (m.useEffect(() => {
      if (T())
        return (
      ...
      if ((C.current++, C.current > k)) {
        (h('Too many initialization attempts, triggering emergency stop'), z());
        return;
      }
      ((o.current = !1), (B.current = !1), s());
      const d = document.getElementById(t);
      if (!d) {
        c('Container element not found:', t);
        return;
      }
      d.querySelectorAll('canvas').forEach(E => {
        E.parentElement &&
          document.contains(E) &&
            ...
            ? setTimeout(() => {
                if (!o.current && !B.current)
                  try {
                    ...
                    (h('Retry failed:', A), C.current >= k && z());
                  }
              }, 200)
            : (h('Max init attempts reached, triggering emergency stop'), z()));
      }
      const $ = T();
      if (
        (_('SiriCallButton'),
        console.log('📱 [SiriCallButton] Device detection - isMobile:', $),
        $)
      )
        return () => {
          M();
      ...
      {
        const E = () => {
            (L(!0), console.log('🖱️ [SiriCallButton] 🟢 DESKTOP Mouse enter'));
          },
          A = () => {
            (L(!1), console.log('🖱️ [SiriCallButton] 🔴 DESKTOP Mouse leave'));
          },
          O = w => {
            (console.log(
              ...
              console.log('🖱️ [SiriCallButton] 🔽 Element ID:', d.id),
              console.log(
                '🖱️ [SiriCallButton] 🔽 isHandlingClick before:',
                v.current
              ));
            const H = d.getBoundingClientRect();
            (I(w, { x: w.clientX - H.left, y: w.clientY - H.top }),
          ...
          V = w => {
            (console.log(
              ...
              console.log('🖱️ [SiriCallButton] 🔼 onCallStart available:', !!i),
              console.log('🖱️ [SiriCallButton] 🔼 isListening state:', l),
              console.log(
                '🖱️ [SiriCallButton] 🔼 isHandlingClick before:',
                v.current
              ),
        ...
        (console.log('🖱️ [SiriCallButton] 🎯 DESKTOP EVENT SETUP:'),
          console.log('  📦 Element ID:', d.id),
          console.log('  📦 Element tagName:', d.tagName),
          console.log('  🎛️ onCallStart available:', !!i),
          console.log('  🎛️ onCallEnd available:', !!n),
          console.log(
            '  🎨 Element computed style:',
            window.getComputedStyle(d).pointerEvents
          ));
        const j = w => {
          (console.log('🎯 [SiriCallButton] 🔥 MANUAL TEST CLICK DETECTED!'),
            console.log('  🎯 Click target:', w.target),
            console.log('  🎯 Click coordinates:', w.clientX, w.clientY),
            console.log('  🎯 Element rect:', d.getBoundingClientRect()),
            console.log('  🎯 onCallStart available:', !!i));
        };
        return (
          d.addEventListener('click', j),
          d.addEventListener('mouseenter', E),
          d.addEventListener('mouseleave', A),
          ...
          () => {
            (console.log(
              ...
              (v.current = !1),
              s(),
              d.removeEventListener('click', j),
              d.removeEventListener('mouseenter', E),
              d.removeEventListener('mouseleave', A),
              d.removeEventListener('mousedown', O),
      ...
      m.useEffect(() => {
        u.current &&
          !o.current &&
          (u.current.setListening(l),
          console.log(
            '🔧 [SiriCallButton] Listening state updated without resize trigger'
          ));
      }, [l, t]),
      m.useEffect(() => {
        u.current && !o.current && u.current.setVolumeLevel(e);
      }, [e]),
      m.useEffect(() => {
        u.current && a && !o.current && u.current.updateColors(a);
      }, [a]));
    const P = async g => {
      if (g.type === 'touchend' || g.type === 'click') {
        if (
          (s(),
          s('  📱 Event type:', g.type),
          s('  📱 Event target:', g.target),
        ...
        try {
          if (!l && i) {
            ...
            try {
              ...
              (h('📱 [SiriCallButton] ❌ Mobile onCallStart() error:', d),
                b('idle'));
            }
          } else
            l && n
      ...
      children: [
        ...
        !x &&
          ...
          p.jsx('div', {
            ...
            style: {
              background: `linear-gradient(135deg, ${a?.primary || '#5DB6B9'}, ${a?.secondary || '#E8B554'})`,
              color: 'white',
              fontSize: '36px',
              boxShadow: `0 0 30px ${a?.glow || 'rgba(93, 182, 185, 0.4)'}`,
        ...
        y !== 'idle' &&
          y !== 'listening' &&
          p.jsx('div', {
            className: `status-indicator ${y}`,
            style: {
              position: 'absolute',
              top: '-48px',
              left: '50%',
              transform: 'translateX(-50%)',
              color: a?.primary || '#5DB6B9',
              textShadow: `0 0 10px ${a?.glow || 'rgba(93, 182, 185, 0.4)'}`,
              pointerEvents: 'none',
            },
            children: y === 'processing' ? 'Processing...' : 'Speaking...',
          }),
      ],
    });
  },
  ...
  ne = ({
    ...
    (console.log(
      ...
      console.log('📏 [SiriButtonContainer] Responsive size:', c),
      m.useEffect(() => {
        l || u(!1);
      }, [l]));
    const y = async x => {
      ...
      b = () => {
        ...
        try {
          ...
          if (
            ...
            const S = x instanceof Error ? x.message : 'Lỗi không xác định';
    ...
    return p.jsxs('div', {
      ...
      children: [
        ...
        p.jsx('div', {
          className: `relative transition-all duration-500 ease-in-out voice-button ${l ? 'listening' : ''} ${h ? 'confirming' : ''}`,
          'data-language': s,
          style: {
            width: c.width,
            height: c.height,
            ...
            boxShadow: h
              ...
              : `0 20px 40px ${o.glow}, 0 0 60px ${o.glow}`,
            background: h
              ? 'linear-gradient(135deg, #80808020, #80808010)'
              : `linear-gradient(135deg, ${o.primary}15, ${o.secondary}10)`,
            backdropFilter: 'blur(10px)',
            border: h ? '2px solid #80808040' : `2px solid ${o.primary}40`,
