        ...
        const interval = setInterval(() => {
            ...
            if (client) {
                ...
                if (isActive !== isCallActive) {
                    ...
                    addLog(isActive ? '📞 Call became active' : '📴 Call became inactive');
                }
            }
        }, 1000);
        ...
        <div className="p-6 bg-white shadow-lg rounded-lg max-w-2xl mx-auto">
            ...
            <div className="mb-4 p-3 bg-gray-100 rounded">
                ...
                <div className="text-sm">
                    <div>Public Key: {config.publicKey ? `${config.publicKey.substring(0, 15)}...` : '❌ Not Set'}</div>
                    <div>Assistant ID: {config.assistantId ? `${config.assistantId.substring(0, 15)}...` : '❌ Not Set'}</div>
                </div>
            </div>
            ...
            <div className="flex gap-2 mb-4">
                ...
                >
                    {isLoading && !isInitialized ? '⏳ Initializing...' : '🚀 Initialize Vapi'}
                </button>
            ...
            {/* Status */}
            <div className="mb-4">
                <div className={`px-3 py-1 rounded text-sm ${isInitialized
                        ? 'bg-green-100 text-green-800'
                        : 'bg-yellow-100 text-yellow-800'
                    }`}>
                    Status: {isInitialized ? (isCallActive ? '📞 Call Active' : '✅ Ready') : '⏳ Not Initialized'}
                </div>
            </div>
