...
export const ServiceGrid: React.FC<ServiceGridProps> = ({
  ...
  const handleServiceClick = useCallback(async (service: ServiceCategory) => {
    logger.debug(`🎯 [ServiceGrid] Service clicked: ${service.name}`, 'Component');
    ...
    try {
      ...
      if (onVoiceServiceRequest) {
        ...
        logger.debug('🎤 [ServiceGrid] Starting voice call for service', 'Component', service.name);
        await startCall();
      }

      logger.debug(`✅ [ServiceGrid] Service request processed: ${service.name}`, 'Component');
    } catch (error) {
      logger.error(`❌ [ServiceGrid] Error processing service: ${service.name}`, 'Component', error);
  ...
  const renderServiceItem = useCallback((service: ServiceCategory, index: number, isMobile: boolean = false) => {
      ...
      return (
        ...
        >
          <div className="text-white flex-shrink-0 relative">
            ...
            <Icon size={32} className={isLoading ? 'opacity-30' : 'opacity-100'} />
          </div>
    ...
    return (
      ...
      >
        ...
        {/* Icon */}
        <div className={`text-white mb-2 ${isLoading ? 'opacity-30' : 'opacity-100'}`}>
          <Icon size={28} />
        </div>

        {/* Text */}
        <div className={`text-white text-center text-sm font-medium ${isLoading ? 'opacity-30' : 'opacity-100'}`}>
          {service.name}
        </div>
