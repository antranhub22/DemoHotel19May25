...
const getNotificationIcon = (type: NotificationType) => {
  ...
  switch (type) {
    ...
    case 'call':
      return <Phone {...iconProps} className="w-5 h-5 flex-shrink-0 text-purple-600" />;
    case 'service':
      return <Settings {...iconProps} className="w-5 h-5 flex-shrink-0 text-indigo-600" />;
    case 'guest':
      return <User {...iconProps} className="w-5 h-5 flex-shrink-0 text-pink-600" />;
    case 'system':
      return <Smartphone {...iconProps} className="w-5 h-5 flex-shrink-0 text-gray-600" />;
    default:
      return <Info {...iconProps} className="w-5 h-5 flex-shrink-0 text-blue-600" />;
  }
};
...
const getNotificationStyles = (type: NotificationType) => {
  const baseClasses = "border-l-4 backdrop-blur-md shadow-lg transition-all duration-300";
...
const getPositionClasses = (position: NotificationSystemProps['position'], isMobile: boolean) => {
  if (isMobile) {
...
const NotificationItem: React.FC<{
  ...
  index?: number;
  isMobile?: boolean;
}> = ({ notification, onClose, isExiting = false, index = 0, isMobile = false }) => {
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <h4 className={`font-semibold text-gray-900 ${isMobile ? 'text-sm' : 'text-sm'} voice-feedback-text`}>
                {notification.title}
              </h4>
              <p className={`text-gray-700 mt-1 ${isMobile ? 'text-xs' : 'text-sm'} leading-relaxed`}>
                {notification.message}
              </p>
              
              {/* Enhanced Metadata Display */}
              {notification.metadata && Object.keys(notification.metadata).length > 0 && !isMobile && (
              ...
              {notification.actions && notification.actions.length > 0 && (
                <div className={`mt-3 flex ${isMobile ? 'flex-col space-y-2' : 'space-x-2'}`}>
...
export const NotificationSystem: React.FC<NotificationSystemProps> = ({
  ...
  const addNotification = useCallback((notification: Omit<Notification, 'id' | 'timestamp'>) => {
    ...
    logger.debug(`📢 [NotificationSystem] Added notification: ${newNotification.title}`, 'Component');
  }, [maxNotifications]);
  ...
  const removeNotification = useCallback((id: string) => {
    ...
    logger.debug(`📢 [NotificationSystem] Removed notification: ${id}`, 'Component');
  }, []);
  ...
  const clearAllNotifications = useCallback(() => {
    ...
    logger.debug(`📢 [NotificationSystem] Cleared all notifications`, 'Component');
  }, [notifications]);
