        ...
        <div className="flex-1 overflow-y-auto mb-3 max-h-60 border rounded p-2 bg-gray-50">
          {messages.length === 0 ? (
            ...
            messages.map(msg => (
              <div
                key={msg.id}
                className={`mb-2 ${msg.sender === 'staff' ? 'text-right' : 'text-left'}`}
              >
                <div
                  className={`inline-block px-3 py-1 rounded-lg ${msg.sender === 'staff' ? 'bg-blue-100 text-blue-900' : 'bg-gray-200 text-gray-800'}`}
                >
                  <span className="font-semibold">
                    {msg.sender === 'staff' ? 'Staff' : 'Guest'}:
                  </span>{' '}
                  {msg.content}
                </div>
