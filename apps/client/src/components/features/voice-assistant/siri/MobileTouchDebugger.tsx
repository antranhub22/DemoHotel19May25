...
export const MobileTouchDebugger: React.FC<MobileTouchDebuggerProps> = ({
  ...
  const testCallStart = async () => {
    logger.debug(
      '🧪 [MobileTouchDebugger] Manual test call start',
      'Component'
    );
    ...
    try {
      if (onCallStart) {
        await onCallStart();
        logger.debug(
          '✅ [MobileTouchDebugger] Manual call start successful',
          'Component'
        );
      } else {
        logger.warn(
          '⚠️ [MobileTouchDebugger] onCallStart not available',
          'Component'
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (!container) {
      logger.warn(
        '[MobileTouchDebugger] Container ${containerId} not found',
        'Component'
      );
      return;
    }
  ...
  if (!enabled || !isMobileDevice() || !debugInfo) {
    return null;
  }
      ...
      {isVisible && (
        ...
        >
          ...
          <div style={{ marginBottom: '15px' }}>
            ...
            {debugInfo.containerInfo.computedStyle && (
              <>
                <div>
                  Pointer Events:{' '}
                  {debugInfo.containerInfo.computedStyle.pointerEvents}
                </div>
                <div>
                  Z-Index: {debugInfo.containerInfo.computedStyle.zIndex}
                </div>
                <div>
                  Touch Action:{' '}
                  {debugInfo.containerInfo.computedStyle.touchAction}
                </div>
              </>
            )}
          </div>
          ...
          <div style={{ marginBottom: '15px' }}>
            ...
            <div>
              isListening: {debugInfo.callbackStatus.isListening ? '🎤' : '🔇'}
            </div>
                ...
                debugInfo.touchEvents
                  ...
                  .map((event, index) => (
                    ...
                    >
                      ...
                      <div>
                        Position: {Math.round(event.position.x)},{' '}
                        {Math.round(event.position.y)}
                      </div>
                      <div>
                        Touches: {event.touches} | Prevented:{' '}
                        {event.prevented ? '✅' : '❌'}
                      </div>
                    </div>
                  ))
          ...
          <div style={{ fontSize: '10px', color: '#999', fontStyle: 'italic' }}>
            Try tapping the Siri button and watch the events above. Use "Test
            Call Start" to verify the callback works.
          </div>
        </div>
      )}
    </>
