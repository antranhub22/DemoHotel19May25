...
export class CanvasRenderer {
  ...
  public drawWaveform(state: CanvasRenderState): void {
    ...
    const { centerX, centerY, radius, waveformPhase, volumeLevel, colors } = state;
    ...
    for (let ring = 0; ring < 3; ring++) {
      ...
      for (let i = 0; i <= points; i++) {
        ...
        const wave = Math.sin(waveformPhase + i * 0.3 + ring * 1.5) * (8 + volumeLevel * 12);
  ...
  public drawRipples(state: CanvasRenderState): void {
    ...
    for (let i = ripples.length - 1; i >= 0; i--) {
      ...
      const alphaHex = Math.floor(255 * ripple.alpha).toString(16).padStart(2, '0');
  ...
  public drawTimeRing(state: CanvasRenderState): void {
    ...
    const color = percent < 0.5 ? colors.secondary : percent < 1 ? colors.primary : '#e53935';
    
    this.ctx.save();
    this.ctx.beginPath();
    this.ctx.arc(
      centerX, centerY, radius * 1.32,
      -Math.PI / 2,
  ...
  public drawIdleFlash(state: CanvasRenderState): void {
    const { centerX, centerY, radius, lastActiveTime, idleFlash, colors } = state;
