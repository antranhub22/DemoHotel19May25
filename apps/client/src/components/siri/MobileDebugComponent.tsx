  ...
  return (
    <div style={{ 
      ...
      <div style={{ marginBottom: '10px' }}>
        ...
        Mobile: {debugInfo.deviceInfo.isMobile ? '✅' : '❌'}<br/>
        Screen: {debugInfo.deviceInfo.screen}<br/>
        Viewport: {debugInfo.deviceInfo.viewport}<br/>
        DPR: {debugInfo.deviceInfo.dpr}
      </div>
      
      <div style={{ marginBottom: '10px' }}>
        <strong>Responsive Size:</strong><br/>
        Width: {debugInfo.responsiveSize.width}<br/>
        Height: {debugInfo.responsiveSize.height}<br/>
        MinWidth: {debugInfo.responsiveSize.minWidth}<br/>
        MaxWidth: {debugInfo.responsiveSize.maxWidth}
      </div>
      
      <div style={{ marginBottom: '10px' }}>
        <strong>Actual Container:</strong><br/>
        Width: {debugInfo.containerSize.width.toFixed(1)}px<br/>
        Height: {debugInfo.containerSize.height.toFixed(1)}px
      </div>
      
      <div style={{ marginBottom: '10px' }}>
        <strong>Canvas:</strong><br/>
        Width: {debugInfo.canvasSize.width.toFixed(1)}px<br/>
        Height: {debugInfo.canvasSize.height.toFixed(1)}px<br/>
      ...
      {debugInfo.alignment.offset && (
        <div style={{ marginBottom: '10px' }}>
          ...
          Offset X: {debugInfo.alignment.offset.x.toFixed(2)}px<br/>
          Offset Y: {debugInfo.alignment.offset.y.toFixed(2)}px<br/>
          Total: {Math.sqrt(debugInfo.alignment.offset.x ** 2 + debugInfo.alignment.offset.y ** 2).toFixed(2)}px<br/>
          Status: {Math.abs(debugInfo.alignment.offset.x) <= 1 && Math.abs(debugInfo.alignment.offset.y) <= 1 ? '✅ Aligned' : '⚠️ Misaligned'}
        </div>
      )}
