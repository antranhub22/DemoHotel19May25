  ...
  const handleConfirm = useCallback(() => {
    ...
    const executeWithFallback = async () => {
      try {
        ...
        try {
          ...
          if (!document.getElementById('spinner-animation')) {
            try {
              ...
              console.warn('⚠️ [useConfirmHandler] Failed to add spinner styles:', styleError);
            }
          }
        ...
        console.log('⏳ [useConfirmHandler] Step 1.5: Brief delay before ending call...');
        await new Promise(resolve => setTimeout(resolve, 300));
        
        if (!isMountedRef.current) {
          console.warn('⚠️ [useConfirmHandler] Component unmounted during delay, aborting');
          return;
        }
        ...
        console.log('🔄 [useConfirmHandler] Step 3: Showing completion message...');
        ...
        setTimeout(() => {
          ...
          try {
            const completionElement = createElement('div', { 
              ...
              transcripts.length > 0 && createElement('div', { 
                key: 'transcript-info',
                style: { 
                  marginBottom: '16px', 
                  padding: '12px', 
                ...
                createElement('div', { 
                  ...
                  style: { fontWeight: '600', marginBottom: '4px', color: '#1e40af' } 
              ...
              serviceRequests?.length > 0 && createElement('div', { 
                key: 'requests',
                style: { 
                  marginBottom: '16px', 
                  padding: '12px', 
                ...
                createElement('div', { 
                  key: 'req-title', 
                  style: { fontWeight: '600', marginBottom: '8px', color: '#15803d' } 
                }, 'Service Requests:'),
                createElement('ul', { 
                  key: 'req-list', 
            ...
            console.log('✅ [useConfirmHandler] Completion message shown successfully');
            
          } catch (completionError) {
            console.error('❌ [useConfirmHandler] Error showing completion message:', completionError);
            // Final fallback - simple alert
            if (isMountedRef.current) {
              setTimeout(() => alert('Call completed successfully! Thank you for using our service.'), 100);
            }
          }
        }, 1000); // Give time for endCall to process
        
        console.log('✅ [useConfirmHandler] Confirm flow completed successfully');
        ...
        if (isMountedRef.current) {
          try {
            ...
            const errorElement = createElement('div', { 
              ...
              createElement('h3', { 
                key: 'title', 
                style: { marginBottom: '16px', color: '#dc2626', fontSize: '18px', fontWeight: '600' } 
              }, '⚠️ Call Processing Issue'),
              
              createElement('p', { 
                key: 'message', 
                style: { marginBottom: '16px', color: '#374151', fontSize: '16px' } 
            ...
            console.error('❌ [useConfirmHandler] Fallback popup also failed:', fallbackError);
            // Ultimate fallback - simple alert after delay
            setTimeout(() => {
              if (isMountedRef.current) {
                alert('Call completed! There was a technical issue with the summary. Please contact front desk for assistance.');
              }
            }, 500);
          }
        }
    ...
    try {
      ...
      setTimeout(() => {
        if (isMountedRef.current) {
          alert('Call completed! Technical issue occurred. Please contact front desk.');
        }
      }, 100);
    }
