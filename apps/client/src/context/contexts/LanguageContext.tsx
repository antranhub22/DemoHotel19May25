...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  const [language, setLanguageState] = useState<Language>(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (savedLanguage && ['en', 'fr', 'zh', 'ru', 'ko', 'vi'].includes(savedLanguage)) {
        logger.debug('[LanguageContext] Loading saved language:', 'Component', savedLanguage);
  ...
  const setLanguage = useCallback((lang: Language) => {
    logger.debug('[LanguageContext] setLanguage called with:', 'Component', lang);
    setLanguageState(lang);
    
    // Persist to localStorage
    if (typeof window !== 'undefined') {
      localStorage.setItem('selectedLanguage', lang);
      logger.debug('[LanguageContext] Language saved to localStorage:', 'Component', lang);
    }
  }, []);

  // Function to translate text to Vietnamese
  const translateToVietnamese = useCallback(async (text: string): Promise<string> => {
    try {
      logger.debug('[LanguageContext] Requesting Vietnamese translation...', 'Component');
      ...
      logger.error('[LanguageContext] Error translating to Vietnamese:', 'Component', error);
