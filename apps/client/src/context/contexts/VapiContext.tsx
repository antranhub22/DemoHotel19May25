...
export function VapiProvider({ children }: { children: React.ReactNode }) {
  ...
  const initializeVapi = async (language: string, hotelConfig?: HotelConfiguration | null) => {
    try {
      logger.debug('[VapiContext] Initializing Vapi for language:', 'Component', language);
      ...
      try {
        ...
        logger.error('[VapiContext] Error getting public key from config:', 'Component', configError);
        publicKey = import.meta.env.VITE_VAPI_PUBLIC_KEY;
      }

      if (!publicKey) {
        throw new Error(`Vapi public key not configured for language: ${language}`);
      }

      if (!publicKey.startsWith('pk_')) {
        throw new Error('Invalid Vapi public key format. Key should start with "pk_"');
      }
      ...
      logger.error('[VapiContext] Error initializing Vapi:', 'Component', error);
      throw error;
    }
  };
  ...
  const setupVapiEventListeners = (vapi: any) => {
    ...
    vapi.on('volume-level', (level: number) => {
      try {
        ...
        logger.warn('[VapiContext] Error handling volume-level:', 'Component', error);
      }
    });
  ...
  const startVapiCall = async (assistantId: string) => {
    try {
      logger.debug('[VapiContext] Starting Vapi call with assistant:', 'Component', assistantId);
      ...
      if (!assistantId.startsWith('asst_')) {
        throw new Error('Invalid assistant ID format. ID should start with "asst_"');
      }
      ...
      logger.debug('[VapiContext] Vapi call started successfully:', 'Component', call?.id || 'unknown');
      return call;
    } catch (error) {
      logger.error('[VapiContext] Error starting Vapi call:', 'Component', error);
      throw error;
    }
  };
  ...
  const setMuted = (muted: boolean) => {
    try {
      ...
      logger.error('[VapiContext] Error setting mute state:', 'Component', error);
    }
  };
