...
export function AssistantProvider({ children }: { children: ReactNode }) {
  ...
  const [vapiCallSummary, setVapiCallSummary] = useState<string | null>(null);
    ...
    const setupVapi = async () => {
      try {
        ...
        const handleMessage = async (message: any) => {
          ...
          if (message.type === 'call-analysis' || message.type === 'analysis-complete' || 
              (message.type === 'message' && message.analysis)) {
            console.log('📊 [Vapi] Call analysis/summary received:', message);
            
            let summaryContent = '';
            
            // Extract summary from different possible formats
            if (message.analysis) {
              summaryContent = message.analysis.summary || message.analysis.content || 
            ...
            if (summaryContent) {
              console.log('✅ [Vapi] Setting call summary:', summaryContent);
  ...
  const startCall = React.useCallback(async () => {
    try {
      ...
      // Reset Vapi call summary for new call
      setVapiCallSummary(null);
      console.log('🔄 [startCall] Reset Vapi call summary for new call');
      ...
      try {
        ...
        try {
          ...
          if (transcriptData.length >= 2) {
            ...
            .catch(async (error) => {
              console.error('❌ [AssistantContext] Error processing OpenAI summary:', error);
              
              // 🔄 FALLBACK: Try to use Vapi call summary if available
              if (vapiCallSummary) {
                console.log('🔄 [Fallback] Using Vapi call summary as fallback');
                ...
                // Wait a bit for Vapi summary to potentially arrive
                console.log('⏳ [Fallback] Waiting for Vapi call summary...');
                setTimeout(() => {
                  if (vapiCallSummary) {
                    console.log('✅ [Fallback] Delayed Vapi summary found, using it');
                    const delayedFallbackSummary: CallSummary = {
                      id: Date.now() as unknown as number,
                      callId: callDetails?.id || `call-${Date.now()}`,
                      content: `${vapiCallSummary}\n\n(Generated by Vapi - OpenAI summary unavailable)`,
                      timestamp: new Date(),
                    ...
                    setCallSummary(delayedFallbackSummary);
                  } else {
                    // Final fallback: show error message
                    console.log('❌ [Fallback] No Vapi summary available, showing error');
                    const errorSummary: CallSummary = {
                      id: Date.now() as unknown as number,
                      callId: callDetails?.id || `call-${Date.now()}`,
                      content: "Unable to generate call summary. Both OpenAI and Vapi summary services are unavailable.",
                      timestamp: new Date(),
                      tenantId: tenantId || 'default'
                    };
                    setCallSummary(errorSummary);
                  }
                }, 3000); // Wait 3 seconds for Vapi summary
              }
            });
          } else {
