...
// ✅ DETAILED MIGRATION: Use Prisma client for staff operations
    ...
    try {
      ...
      const finalRequests = await dashboardCache.get(
        ...
        async () => {
          ...
          // ✅ DETAILED MIGRATION: Use Prisma for tenant-filtered requests
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        // ✅ DETAILED MIGRATION: Use Prisma for request lookup
        requestDetails = await prisma.request.findUnique({
          where: {
            id: parseInt(id),
          },
        });
