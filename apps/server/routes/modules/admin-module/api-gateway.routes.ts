...
router.get("/rate-limits", async (req: Request, res: Response) => {
  try {
    ...
    const gateway = APIGateway.getInstance();
    const _diagnostics = gateway.getDiagnostics();
  ...
  try {
    ...
    const summary = {
      ...
      healthyTargets: routes.reduce(
        (sum, r) =>
          sum + r.targets.filter((t) => t.health === "healthy").length,
        0,
      ),
...
router.get("/cache", async (req: Request, res: Response) => {
  try {
    ...
    const _diagnostics2 = gateway.getDiagnostics();
...
router.get("/security", async (_req: Request, res: Response) => {
  try {
    ...
    const gateway = APIGateway.getInstance();
    const metrics = await gateway.getMetrics();
    const _diagnostics3 = gateway.getDiagnostics();
...
router.get("/config", async (_req: Request, res: Response) => {
  try {
    ...
    const gateway = APIGateway.getInstance();
    const _diagnostics4 = gateway.getDiagnostics();
