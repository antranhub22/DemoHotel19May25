...
function compareSchemas(prismaModels, dbSchema) {
    ...
    const commonTables = prismaTableNames.filter(name => dbTableNames.includes(name));
    ...
    const hasMigrationScripts = Object.keys(packageJson.scripts).some(script =>
...
async function main() {
    ...
    if (databaseUrl && databaseUrl.includes('postgres')) {
        ...
        if (dbSchema) {
            console.log(`✅ Connected to database, found ${dbSchema.tables.length} tables`);
        ...
        console.log('💡 To test with PostgreSQL, set DATABASE_URL to a PostgreSQL connection string');
    ...
    if (databaseUrl && databaseUrl.includes('postgres')) {
        ...
        console.log('💡 Set DATABASE_URL to PostgreSQL connection to test schema consistency');
...
module.exports = { main, readPrismaSchema, checkPostgreSQLSchema, compareSchemas }; 