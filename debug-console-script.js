...
async function debugVapiLanguages() {
    ...
    for (const lang of LANGUAGES) {
        try {
            ...
            console.log(`${status} ${lang.toUpperCase()}: Assistant ID: ${assistantId}`);
...
function checkBrowserCache() {
    ...
    // Check localStorage
    const vapiKeys = Object.keys(localStorage).filter(key => 
        key.includes('vapi') || key.includes('language') || key.includes('assistant')
    );
...
async function runFullDebug() {
    ...
    console.log('💡 If all APIs show SUCCESS but language selection still fails:');
